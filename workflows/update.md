...
On **apply** (1), append the Lax footer (see CLAUDE.md) to any comment and to any updated description just before sending — blank line, then `*Done with [Lax](https://github.com/styoe/lax)*`. Skip the append if the body already contains the substring `github.com/styoe/lax` (case-insensitive — matches renderer-mangled variants too). Then write via the configured ticket provider's MCP server and append a log entry to `log.jsonl` (with the current `session_id`, action, ticket_id, comment_id if any, fields_written, result). Do not write before approval.
